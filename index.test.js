...
beforeAll(async () => {
    ...
    await setupDB.schema.withSchema("public").createTable('default_table', table => {
        table.increments();
        table.enu('cust_type', ['value1', 'value2'], { useNative: true, enumName: 'cust_type' })
        table.string('name');
        table.boolean("flag")
        table.timestamps();
    ...
    await setupDB.schema.raw("CREATE VIEW public.default_view AS select * from public.default_table");
    ...
    await setupDB.schema.withSchema("not_default").createTable('not_default_table', table => {
        table.increments();
        table.enu('cust_type_not_default', ['custom1', 'custom2'], { useNative: true, existingType: true, enumName: 'cust_type_not_default', schemaName: "not_default" })
        table.string('name_2');
...
test('in default schema', async () => {
    ...
    expect(extracted.types.filter((t)=> t.name === "cust_type_not_default")).not.toBeNull();
});
